...
export function LoginPage() {
  ...
  const handleSendOtp = async (e: FormEvent) => {
    ...
    if (cleaned.length !== 10) {
      setPhoneError(t('auth.validPhoneError'));
      return;
    }
    setPhoneError('');
    setIsLoading(true);
    try {
      ...
      setOtpSent(true);
      setResendTimer(30);
      addToast({ type: 'success', title: t('auth.otpSent') });
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 pb-12">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-earth-900">{t('auth.loginHeading')}</h1>
          </div>
          ...
          {method === 'phone' && otpSent && (
            <div className="space-y-4 animate-fade-in-up">
              <p className="text-center text-sm text-earth-600">{t('auth.enterOtp')} — +91 {phone}</p>
              ...
              <div className="text-center">
                {resendTimer > 0 ? (
                  <p className="text-sm text-earth-400">{t('auth.resendOtpIn')} <span className="font-semibold text-earth-600">{resendTimer}s</span></p>
                ) : (
                  <button
                    type="button"
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setConfirmationResult(result);
                        setResendTimer(30);
                        addToast({ type: 'success', title: t('auth.otpResent') });
                        otpRefs.current[0]?.focus();
                      } catch (err) {
                        addToast({ type: 'error', title: friendlyError(err) });
                      } finally {
